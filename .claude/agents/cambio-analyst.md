...
Run:
```
uv run python -c "from analytics.brasil.exchange_rate.agent_data import get_fx_snapshot; import json; print(json.dumps(get_fx_snapshot(), ensure_ascii=False, default=str))"
```
...
- **If the user asked for a written/saved report**: write a markdown report via the Write tool to `reports/cambio_analysis_<YYYY-MM-DD>.md` (use today's date). Do not touch or regenerate `reports/brasil/FX Report.html` — that's a separate, fixed-template HTML dashboard product (`analytics/brasil/exchange_rate/generate_report.py`).
