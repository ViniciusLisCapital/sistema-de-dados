...
domain/
  db/brasil/         — ETL Brasil: fetch → transform → insert em macro_brasil
    ...
    bcb/             — Scripts por tema BCB (ibc_br, inflacao, caged, credito, expectativas, indicadores_familias)
  db/cambio/         — ETL Câmbio: fetch → insert em macro_cambio  [ver CAMBIO.md]
    bcb/             — reservas, balanco_pagamentos, fluxo_cambial, termos_de_troca
...
analytics/           — Projetos que consomem o banco MySQL
  ...
  cambio/            — Panorama Cambial HTML  [ver CAMBIO.md]
    generate_report.py  — Lê macro_cambio, injeta JSON no template, salva HTML
...
jobs/                — Entry points
  ...
  update_cambio.py   — Atualiza todas as tabelas de macro_cambio
...
- **macro_cambio — itens menores**: confirmar descrições SGS 22099/22100, sub-itens CEP/CBE fluxo cambial, swap cambial BCB. Ver `CAMBIO.md`.
