...
| `brasil/monetary_policy/` | BCB small-model replication (HTML report) | documented inline in root `CLAUDE.md` + [`brasil/monetary_policy/referencia/MODEL_REPLICATION_PLAN.md`](brasil/monetary_policy/referencia/MODEL_REPLICATION_PLAN.md) |
...
| **`us/`** | | |
| `us/` | Nothing yet — created 2026-08 with the country-first rename. First report to build: `us/inflation/` (CPI; the connector and the item hierarchy are already done — see [`connectors/bls.py`](../connectors/bls.py) and [`us_project/inflation_hierarchy.md`](../us_project/inflation_hierarchy.md)) | — |
| **cross-country (root)** | | |
