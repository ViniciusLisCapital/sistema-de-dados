...
def _load_pib_pm_raw() -> dict:
    ...
    eixos de metrica -- ver analytics/fiscal_policy/transforms.py's docstring).
    """
    pib = _load_table("atv_pib_valores_correntes")
    pib_pm = pib[pib["name"] == "pib_pm"].set_index("date")["value"].sort_index()
...
def _load_rtn_tab_data() -> dict:
    ...
    analytics/fiscal_policy/rtn_tab.py e reference/rtn_vs_efgg.md."""
...
def _impulso_quarter_via_stl(flow: dict, gdp_by_date: dict, target_dates: list[str]) -> list:
    ...
    # ver Gotchas em analytics/fiscal_policy/CLAUDE.md): pctgdp comeca com 2 meses None
...
def _load_fiscal_impulse_nfsp() -> dict:
    ...
    dois fica para uma proxima rodada, ver Pending em analytics/fiscal_policy/CLAUDE.md.
...
def _load_pib_yoy() -> dict:
    ...
    atv_pib_taxas (mesma tabela que analytics/economic_activity usa para o mesmo fim, ver Data
