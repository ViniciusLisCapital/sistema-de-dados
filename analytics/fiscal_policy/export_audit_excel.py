...
ver analytics/fiscal_policy/CLAUDE.md, Gotchas).

Uso:
    uv run python -c "from analytics.fiscal_policy.export_audit_excel import run; run()"
...
from analytics.fiscal_policy.generate_report import (
...
def _build_readme_sheet(wb: Workbook) -> None:
    ...
    lines = [
        ...
        ("", None, False),
        ("Gerado por analytics/fiscal_policy/export_audit_excel.py — espelha exatamente os", 10, False),
        ...
        ("decisões de escopo: analytics/fiscal_policy/CLAUDE.md (Gotchas + corpo principal).", 10, False),
    ]
