# analytics/labor_market/ — Panorama de Mercado de Trabalho
...
```powershell
uv run python -c "from analytics.labor_market.generate_report import run; run()"
...
`analytics.fiscal_policy.transforms`) and `variants_caged_*` for CAGED (rolling/YTD sums).
...
same convention `analytics/credit/CLAUDE.md` already documents for its own no-native-total cuts. 26 such
...
- **No browser has been used to visually confirm** table/expand/checkbox/toggle/chart interactions —
  ...
  stub-`document` approach `analytics/credit/` uses — `buildTableBlock()` sets `innerHTML` and then
