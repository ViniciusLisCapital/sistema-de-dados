...
[`analytics/brasil/labor_market/fontes_dados.md`](../../brasil/labor_market/fontes_dados.md). Cross-cutting
access status: [`us/CLAUDE.md`](../CLAUDE.md). Brazil counterpart: the BCB small-model replication in
[`analytics/brasil/monetary_policy/`](../../brasil/monetary_policy) plus `expc_focus`.

This is the best-covered branch of the eight: **everything essential is free and reachable**, and
almost all of it is on FRED. No API key is missing here. **Partly built since this survey**: the Treasury
curve and the Fed target are in `macro_us.us_interest_rate` (2026-09-03); everything else below is
still only surveyed. Also present from before: `fed_funds` inside
`macro_international.diferenciais_juros`, and two financial-conditions series read ad hoc by
`analytics/oraculo/us/term_us.py`, marked `(oráculo)`.
...
| Effective policy rates | **NY Fed** / Fed Board | ✅ `EFFR`, `DFF`, `FEDFUNDS` (monthly, 1954-07→), `OBFR`, `IORB` (2021-07→), `DPCREDIT` | ✅ **NY Fed markets API** (JSON) | FRED / JSON | ⚠️ `fed_funds` in `macro_international`; the Fed **target** (not the effective rate) is in `macro_us.us_interest_rate`, curve `POLICY` | The NY Fed API returns SOFR/EFFR/OBFR with percentiles and volumes — richer than FRED's single rate |
...
| Treasury nominal yield curve | **Treasury** / Fed Board H.15 | ✅ all 10 tenors: `DGS1MO`, `DGS3MO`, `DGS6MO`, `DGS1`, `DGS2`, `DGS3`, `DGS5`, `DGS7`, `DGS10`, `DGS20`, `DGS30` | ✅ **Treasury CSV** (14 tenors, incl. 1.5m/2m/4m) | FRED / csv | ✅ `macro_us.us_interest_rate`, curve `US_TREASURY`, 11 tenors 1M-30Y (2026-09-03) | The Treasury CSV is one request per calendar year and carries **four tenors FRED does not** |
...
- **Decide whether the policy-expectation path matters.** The Brazil model's known calibration gap is
  ...
  forward curve (see [`analytics/brasil/monetary_policy/`](../../brasil/monetary_policy)). For the US, the
