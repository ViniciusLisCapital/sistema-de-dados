...
access status and outstanding API keys: [`us/CLAUDE.md`](../CLAUDE.md). Brazil counterpart:
[`analytics/brasil/inflation/CLAUDE.md`](../../brasil/inflation/CLAUDE.md).
...
**Built since this survey.** CPI and PCE are both in `macro_us` — `inflc_cpi`, `inflc_cpi_dim`,
`inflc_cpi_pesos`, `inflc_pce`, `inflc_pce_dim` — and the report is `reports/us/Inflation.html`
(see [`CLAUDE.md`](CLAUDE.md) in this folder). The coverage table below has been kept current for
those; the rest of the branch (PPI, import/export prices, expectations) is still only surveyed.
Separately, `macro_international.diferenciais_juros` holds `cpi_12m_us` as an input to the BR-US
real-rate differential, and five series are read ad hoc by `analytics/oraculo/us/term_us.py`,
marked `(oráculo)`.
