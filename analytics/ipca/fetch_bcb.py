...
Uso:
    uv run python analytics/ipca/fetch_bcb.py
    uv run python -c "from analytics.ipca.fetch_bcb import run; run()"
"""
...
_SAAR_SERIES = {
    ...
    "IPCA_nucleo_medias_aparadas", "IPCA_nucleo_EX0", "IPCA_nucleo_EX03",
...
def _apply_stl_ma3(df: pd.DataFrame) -> pd.DataFrame:
    """Compute MA(3) for each SAAR series, apply out-of-sample STL, store as _ma3_sa.
    ...
    for name, grp in df.groupby("name"):
        ...
        # Step 1: MA(3) on full series
        ma3 = vals.rolling(3).mean()

        # Step 2: fit STL only on in-sample MA(3)
        ma3_in = ma3[in_mask].interpolate(method="linear").ffill().bfill()
        if len(ma3_in) < 24:
            continue

        try:
            fit = STL(ma3_in.values, period=12, robust=True).fit()
        except Exception as e:
            print(f"  STL failed for {name}: {e}")
            continue

        # Step 3: average seasonal component by calendar month (frozen factors)
        ...
        # Step 4: apply frozen factors to full series
        seasonal = pd.Series(
...
def run(start: str = "01/01/2000") -> pd.DataFrame:
    ...
    print(f"  Applying MA(3) + STL to {len(_SAAR_SERIES)} series...")
