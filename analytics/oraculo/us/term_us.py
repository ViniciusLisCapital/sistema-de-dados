...
from connectors.fred import FredMultFrame, FredUniFrame, US_IndexNormalize
...
def Jobs_Unemp():
    ...
    #* Gerando as notas
    Trend_Jobs_Unemp = GetScore(BaseJobsperUnempl, 1, Trend_ColumnName, direction=1)
...
def run() -> pd.DataFrame:
    """Atualiza US_BASE.csv com todas as notas macro do oraculo EUA."""
    frames = [
        UnempRate(),
        Jobs_Unemp(),
        PayRoll(),
    ...
    df = concat(frames)
    path = Path(__file__).resolve().parent / 'base' / 'US_BASE.csv'
    df.to_csv(path, index=False)
    print("Calculo de notas US: Executado com sucesso!")
    return df
