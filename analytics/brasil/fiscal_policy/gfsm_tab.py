...
Python (ver analytics/brasil/fiscal_policy/transforms.py) -- mesmo padrao "tabela
hierarquica + grafico" ja usado por analytics/brasil/credit/'s saldo_tab.py/concessao_tab.py
...
def build(raw: dict, ipca_pct: dict, gdp_ttm: dict, gdp_same_period: dict) -> dict:
    ...
    analytics/brasil/fiscal_policy/transforms.py) -- denominador do %PIB no nivel Trimestral.

    Retorna `series` chaveada por "{esfera}__{code}" (dois underscores, mesma
    convencao "{table}__{modalidade}" de analytics/report_structure/tree_helpers.py) -- o lado
    ...
    analytics/brasil/fiscal_policy/transforms.py): `series[key]["bruto"]` (compute_variants(),
