...
de analytics/brasil/credit/transforms.py (STL + deflacao IPCA + %PIB pre-computados em Python, o
...
outras):
  - Bruto (compute_variants()/RTN via analytics.brasil.credit.transforms.compute_variants()):
...
analytics/brasil/fiscal_policy/CLAUDE.md para a duvida em aberto sobre se a variante Real
...
convencao de analytics/brasil/credit/transforms.py.
...
def stl_seasonal_adjust(dates: list[str], values: list, period: int = 4, min_obs: int | None = None) -> list:
    ...
    Divergencia deliberada de analytics/brasil/credit/transforms.py (que congela a MEDIA do fator
...
def ytd_sum(dates: list[str], values: list) -> list:
    ...
    Deliberadamente NAO reusa `analytics.brasil.labor_market.transforms.ytd_sum()`, que tem a
...
def compute_variants_monthly_ttm(
    ...
    "Acumulado" oferecida ao lado da bruta (analytics.brasil.credit.transforms.compute_variants(),
    ...
    convencao "3M/3M rolante" ja usada em analytics/brasil/credit/). Nomes de chave mantidos
