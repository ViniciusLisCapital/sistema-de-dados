...
**Every input is a MySQL table, and this report has no `procedures:` step — since 2026-09-11.**
The four tables it reads (`inflc_decomposicao`, `inflc_dim`, `inflc_decomposicao_item`,
`inflc_agregados`) are all declared in the IPCA/IPCA-15 groups of
[`calendar_2026.yaml`](../../../domain/release_calendar/calendar_2026.yaml), so the calendar's
**Atualizar** button runs all of them (measured: 15s for the three that carry a date), and
**Regerar** only has to build the HTML. That is the point of the arrangement — each button does its
job completely, with nothing in between that only a person remembers.

**What it replaced, and why it mattered.** Until then the BCB/SGS aggregates came from
`data/ipca_bcb_series.csv`, which the report fetched from the SGS itself through a declared
`procedures:` step. That file was a **duplicate of `inflc_agregados`** — a table that already
existed, was already in `jobs/update_db.py`, and was already current. The duplication broke both
buttons at once: no ETL moved the file, so **Atualizar** never reached it; and the step's verdict
read `MAX(dt)` over the *whole* CSV with every series pooled into one bucket, so the IPCA-15 rows —
which arrive about two weeks before the full IPCA — carried the maximum to the new month and hid
that all 33 full-IPCA aggregates were a month behind. Green on screen, stale núcleos and difusão in
the report, which is how a user found it on 2026-09-11. Verified before deleting the file:
**10,556 observations compared between CSV and table, zero difference**; and after rewiring,
54 series and 17,272 values through the new path, also zero difference — including the STL-derived
`_ma3_sa` ones. The one series the table lacked, `ipca_12m` (SGS 13522), was added to the ETL and
backfilled (549 months from 1980-12, against 320 from 2000-01 in the CSV).
...
- `_load_bcb()`: reads `macro_brasil.inflc_agregados`, renames the table's lowercase series to the display names `report.html` expects (`_BCB_RENAME`, 34 entries), cuts at `_BCB_INICIO` (`2000-01` — the sample this report has always shown; the table reaches back to 1980 and extending it is a decision, not a side effect of changing source), and derives the 20 `_ma3_sa` variants with `_apply_stl_ma3()` at generation time. **It raises on a series the rename map does not cover** — adding a series to the ETL and forgetting the map now breaks the build instead of silently dropping that series from the report, which is how a whole núcleo could vanish before.
- `_compute_ipca15_nucleos()`: derives EX-0/EX-01/EX-02/EX-03/EX-FE (+ EX-03 subcomponents) for IPCA-15 from `_load_decomposicao()`'s already-merged dataframe (not a third MySQL read) — see Gotchas. Runs through `_apply_stl_ma3()` (the same function the BCB series use, now in this module) and gets merged into the same `data["bcb"]` dict `_load_bcb()` populates, so front-end code treats it exactly like a BCB series.
...
| MySQL `inflc_decomposicao_item` | IPCA/IPCA-15 by item_codigo (4-digit, one level coarser than `inflc_decomposicao`'s subitem/7-digit): var_mensal/pesos/contribuicao. Same `VIGENCIAS`/agregados as `inflc_decomposicao`, just `len(code) == 4` instead of `== 7`. Feeds only MA/MS/DP (see Gotchas for why these need item, not subitem, level) | `domain/db/brasil/ibge/inflc_decomposicao_item.py` | `jobs/update_db.py` e o Atualizar do calendário (entrou na rotina em 2026-09-11; antes só o botão o alcançava, e a tarefa agendada deixava MA/MS/DP envelhecer) / `backfill()` (histórico completo, manual) |
| MySQL `inflc_agregados` | BCB/SGS aggregates (headline, components, núcleos, difusão, `ipca_12m`), 34 series. The `_ma3_sa` STL variants are **derived at generation time**, not stored | `domain/db/brasil/bcb/inflc_agregados.py` | `jobs/update_db.py` and the calendar's Atualizar (IPCA/IPCA-15 groups) |
...
- **BCB/SGS aggregates used to live in two places that were NOT kept in sync — resolved 2026-09-11** (see "Generate the report"). `macro_brasil.inflc_agregados` is the only copy now; `data/ipca_bcb_series.csv` and `fetch_bcb.py` are gone. The lowercase→display rename lives in `generate_report.py::_BCB_RENAME` and raises on an unmapped series. **The transferable lesson is about the verdict, not the file**: a staleness check that pools every series of a file into one `MAX()` is not a staleness check — one series that moves early certifies as current a file whose other 33 are a month old, and nothing on any screen looks wrong.

- **The IPCA-15 headline was the one series on the núcleo charts that lagged the release by a day — fixed 2026-08-26, found by a user report.** Every other IPCA-15 series on "Componentes/Núcleos — 3M SAAR" and the heatmap is computed in-house from `inflc_decomposicao`, so it lands the morning IBGE publishes; `IPCA15` itself came only from `data/ipca_bcb_series.csv` (SGS 7478), and **SGS mirrors the IBGE release with about a day of lag**. On release day that left `IPCA15_ma3_sa` one month short of the núcleos drawn beside it, with two visible symptoms and one silent: the dotted "IPCA-15 (ref.)"/"(total)" line stopped a month early, and — worse, because nothing about it looked wrong — **the entire heatmap lost its newest column for every row**, since its 12 columns are `compute3mSAAR('IPCA15').dates.slice(-12)`. `generate_report.py::_splice_headline_15()` now feeds `IPCA15` into `series15` as SGS-where-it-exists + `_compute_headline()` reconstruction for the not-yet-mirrored tail, so published history stays byte-identical to the official print (verified: 0 difference across all 315 SGS months) and only the newest month carries IBGE's subitem rounding (0.006 p.p. mean / 0.067 p.p. worst case over those 315 months, under 0.005 p.p. in the last 18 — below the 1-2 decimals anything displays), replaced by the official value on the next `inflc_agregados` load. Side effect: the "12 Meses" KPI for IPCA-15 now resolves at tier 2 of the fallback below instead of falling through to the tier-3 subitem reconstruction.
...
- **The BCB aggregates went silently stale for a full month three times** (before 2026-08; caught
  2026-09-01; caught again 2026-09-11) and every time the only symptom was a chart quietly one
  month short. Closed at the root on 2026-09-11 by deleting the second copy rather than by
  scheduling its refresh — an input with no owner keeps finding new ways to lag, and the fix that
  lasts is not having one.


- **Núcleos por exclusão for IPCA-15 are computed in-house, not published by BCB.** BCB only publishes núcleo series (EX-0/EX-01/…/P55/MA/MS/DP) for the full IPCA; `inflc_agregados` only carries the IPCA-15 headline (SGS 7478). Since 2026-07, `generate_report.py::_compute_ipca15_nucleos()` derives EX-0/EX-01/EX-02/EX-03/EX-03-Serviços/EX-03-Industriais/EX-FE for IPCA-15 directly from `inflc_decomposicao` + `inflc_dim`'s official `nucleo_*` membership flags (same NT-57 vector already used for Difusão-by-núcleo) — weighted average of `contribuicao`/`pesos` among flagged subitems each month, then run through the same STL+MA3 pipeline as BCB series (`_apply_stl_ma3`, which takes an explicit `series` set instead of only the module-level `_SAAR_SERIES`) so the front-end's existing `compute3mSAAR`/`computeYoY` work unmodified — the computed series are merged into `data["bcb"]` under names like `IPCA15_nucleo_EX0` (see `NUCLEO_ALL_15` in `report.html`). **P55, Médias Aparadas (MA/MS), and Dupla Ponderação remain out of reach this way** — they need the full statistical methodology (percentile, tail-trimming, volatility weighting), not just a fixed exclusion filter; see Pending.
...
Os dois graficos cujo X **nao** e tempo (inercia x peso, momentum x nivel) passam `periodo`
pronto: derivar a janela das abscissas imprimiria uma faixa de valores na linha da fonte,
lida como se fosse periodo. Em compensacao a unidade deles nomeia os DOIS eixos.
...
- ~~Migrate the BCB aggregates to read from `macro_brasil.inflc_agregados`~~ and ~~get them onto a scheduled run~~ — both done 2026-09-11, and by the same change: the CSV was deleted rather than scheduled. See "Generate the report".
