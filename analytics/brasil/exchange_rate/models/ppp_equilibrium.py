...
the dashboard first, decide on the regression afterward):
  ...
  sp500   macro_international.cmb_equity_us.sp500 (Yahoo Finance ^GSPC, S&P
          ...
          analytics/brasil/exchange_rate/CLAUDE.md for the full comparison. Read
  ...
  trade_pct_gdp / ca_pct_gdp  Trade balance and current account, both as %
          ...
          analytics/brasil/exchange_rate/generate_report.py's own "% PIB" toggle
...
fused into analytics/brasil/exchange_rate/report.html, so there is one FX report and
one entry point. run() here is now diagnostics-only.

Usage:
    uv run python -c "from analytics.brasil.exchange_rate.models.ppp_equilibrium import run; run()"
"""
...
def run() -> dict:
    ...
    analytics.brasil.exchange_rate.generate_report.run().
    """
    df = load_data()
    ...
    print(
        ...
        'analytics.brasil.exchange_rate.generate_report import run; run()"'
    )
