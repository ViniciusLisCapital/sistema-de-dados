...
[`analytics/brasil/credit/CLAUDE.md`](CLAUDE.md) para a arquitetura do relatório (que hoje só consome uma
...
| PTC — Pesquisa Trimestral de Condições de Crédito (16 séries: 4 segmentos × oferta/demanda × observada/esperada) | 21380-21395 | ✅ `cred_ptc` (tabela própria) → aba **PTC** do relatório | Percepção dos bancos sobre condições de crédito (equivalente ao Senior Loan Officer Opinion Survey do Fed), **não** dado de crédito bancário no sentido desta planilha. **Média aritmética simples (não ponderada) das respostas em escala −2..+2** — não é saldo líquido, e o intervalo é −2 a +2 (o `COMMENT` da tabela dizia ±1 — corrigido em 2026-08, com a escala no `COMMENT` da coluna `value`). O relatório deriva dessas 16 séries uma 17ª leitura, a **surpresa** `observada(t) − esperada(t−1)` — subtração, nada modelado; o BCB não publica essa série. Metodologia: BCB TD 245, Annibal & Koyama (2011), `https://www.bcb.gov.br/pec/wps/port/TD245.pdf` (a nota de rodapé dos relatórios trimestrais cita "TD 254" — erro de digitação do BCB). **Correção 2026-08**: os códigos usados antes aqui (21397/21399, só "Aprovação Observada" de 2 dos 4 segmentos) e em `analytics/brasil/painel_setores/painel_setores.py` (21397/21399/21401/21403, os 4 segmentos) estavam **obsoletos, não descontinuados** — o BCB manteve a pesquisa rodando sob outros códigos (confirmado ao vivo contra a planilha oficial `Series_PTC.xlsx` do BCB e por chamada direta à API); os códigos antigos simplesmente pararam de ser atualizados em 2022-10 enquanto a série real seguia até hoje. Migrado para tabela própria (`cred_ptc`, 16 séries — as 2 direções × 2 horizontes × todos os 4 segmentos, não só "Aprovação Observada" de 2), colunas antigas removidas de `cred_inadimplencia_pj` e `painel_setores.py` corrigido para os códigos certos. Ver `domain/db/brasil/bcb/cred_ptc.py` para o detalhe completo. |
...
- **Nenhuma tabela nova foi ainda conectada ao relatório `analytics/brasil/credit/`** — o escopo desta rodada
  foi só organizar os dados no MySQL; ver "Pending" em
  [`analytics/brasil/credit/CLAUDE.md`](CLAUDE.md) para o que fica disponível para uma futura expansão do
