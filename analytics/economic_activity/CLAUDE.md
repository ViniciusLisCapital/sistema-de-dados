...
- **Heatmap click-to-expand needs two listeners**: `plotly_click` (cell clicks only) *and* a DOM-level
  ...
  `_tabHeatmapCtx` cache (rebuilt every render, never captured by closure). `analytics/exchange_rate/
...
- Open `reports/economic_activity_latest.html` in a real browser — the one verification step the sandbox
...
- `analytics/exchange_rate/report.html`'s BOP heatmap likely needs the same click-listener backport (see
