...
| Modelo estrutural | Uma lista por equação. [`…/structural_model/pendencias_philips_eq.md`](analytics/brasil/structural_model/pendencias_philips_eq.md) — a curva de Phillips: variáveis a acrescentar (Brent, ONI, parte livre em monitorados), testes que faltam e o passo de parâmetros variando no tempo. [`…/pendencias_expectativas_eq.md`](analytics/brasil/structural_model/pendencias_expectativas_eq.md) — a equação de expectativas: tudo o que foi medido e **não** entrou na forma, com o número de cada coisa. [`…/pendencias_is_eq.md`](analytics/brasil/structural_model/pendencias_is_eq.md) — a curva IS: o hiato em tempo real (deixado de fora por decisão do usuário), as candidatas a taxa de juro de equilíbrio medidas uma a uma, e o prêmio de maturidade da NTN-B. [`…/pendencias_taylor_eq.md`](analytics/brasil/structural_model/pendencias_taylor_eq.md) — a regra de juros: as janelas testadas para a taxa de equilíbrio, as três leituras do desvio da inflação, e a intenção de estimar a neutra em vez de escolher um proxy (R12, que é a mesma coisa que H9 na curva IS). [`…/pendencias_fx_eq.md`](analytics/brasil/structural_model/pendencias_fx_eq.md) — o câmbio trimestral: a bolsa americana trocando de sinal entre a correlação bruta e o coeficiente (vale para o FX Report publicado também), as duas convenções de trimestralização, que voltam a ser decisão no dia em que a segunda equação entrar no simulador, e a volatilidade implícita que falta |
