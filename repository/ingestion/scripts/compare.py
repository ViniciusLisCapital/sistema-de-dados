"""
repository/ingestion/scripts/compare.py — Optional semantic quality spot-check.

Uses a configurable Claude model to verify that a clean_md preserves all
substantive content from its raw_md source. Boilerplate removal and
formatting cleanup are both expected and correct. Not called automatically by
run.py (which is deterministic and free) — this is a manual, optional,
paid ($0.01/doc) extra check for when you want a second opinion.

── Programmatic use ──────────────────────────────────────────
    from compare import generate_report
    generate_report(pairs, model=VERIFY_MODEL, out_path=Path("repository/ingestion/compare_report.md"))
    # pairs: list of (raw_path, clean_path)

── Standalone (ad-hoc comparison) ──────────────────────────────────────────
    uv run python repository/ingestion/scripts/compare.py <raw.md> <clean.md>
    uv run python repository/ingestion/scripts/compare.py  # uses hardcoded FILES list below

Options:
    --model   Claude model for verification (default: claude-haiku-4-5-20251001)
    --output  Output .md path (default: repository/ingestion/compare_report.md)
...
def generate_report(
    ...
    if out_path is None:
        out_path = root / "repository" / "ingestion" / "compare_report.md"
...
def main() -> None:
    ...
    root = Path(__file__).resolve().parent.parent.parent.parent
    out_path = Path(args.output) if args.output else root / "repository" / "ingestion" / "compare_report.md"
