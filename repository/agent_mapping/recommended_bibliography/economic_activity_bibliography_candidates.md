...
**Purpose:** first-pass candidate literature list for the economic activity agent, built following `BIBLIOGRAPHY_METHODOLOGY.md`. Nothing here has been acquired yet — this is the pre-acquisition step. Once a source is sourced into `repository/economic_activity/` (naming convention: `topic_description (Author, Year).pdf`), process it into a new `economic_activity_conceptual_map.md`, one source at a time, following the same pattern used for `exchange_rate_conceptual_map.md`.
