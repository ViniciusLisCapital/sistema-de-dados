...
One subfolder per **source**, not per theme: `brasil/` has `ibge/`, `bcb/`, `tesouro/`, `mdic/`, `mte/`, `ipea/` (só `cmb_termos_troca` — séries que o SGS não tem) and `bloomberg/` (só `cmb_risco_pais` — planilha exportada à mão, o único script aqui sem connector; substituiu `investing/` em 2026-09-08, que ficou como pasta morta com os CSVs antigos e um README); `international/` has `bis/`, `cftc/`, `fred/`, `noaa/`, `yfinance/`; `us/` has `inflation/`, `labor_market/` and `rates/`. A table's schema follows the rule below, not the folder it's loaded from — `cmb_termos_troca` comes from IPEA and still lives in `macro_brasil`.
...
br_     — the ONE country-prefixed name in this schema, and deliberate: br_interest_rate.
          The three term-structure tables were all called `interest_rate`, one per schema,
          and registry.py maps table -> module WITHOUT the schema, so the day a second one
          got a loader they would silently fight over the same key (`mapa[tabela] = dotted`,
          last scanned wins). Renamed 2026-09-03 at the user's request to br_/us_/inter_,
          which fixes the collision at the root instead of papering over it with _OVERRIDES.
          Do not "restore the convention" here without re-reading that
```

`macro_international`:
```
cmb_    — FX: cmb_reer, cmb_cot_fx, cmb_dollar_index, cmb_dollar_index_em, cmb_real_rates
...
inflc_  — CPI (levels, the two item trees, relative importance) and PCE (price index +
          ...
          agency publishes it
expc_   — inflation EXPECTATIONS (expc_inflacao, the Cleveland Fed term structure). Same
          prefix as macro_brasil's Focus tables, and deliberately NOT inflc_: an
          expectation is not a price index, same rule as above
mt_     — labor market. JOLTS (mt_jolts + mt_jolts_dim), CES (mt_ces + mt_ces_dim),
          CPS (mt_cps) — the three monthly BLS surveys — and mt_produtividade, the
          quarterly Productivity and Costs release; the same
...
| `mt_desocupacao_retro` | BCB, anexo estatístico do RPM (**não existe no SGS** — ver `connectors/bcb_rpm.py`): `Graf 1.2.x — Taxa de desocupação`, série única. **%, MM3M, dessazonalizada**, retropolada pelo próprio BCB (Alves e Fasolo, WP 400/2015) no trecho anterior à PNAD Contínua. É a série de emprego do modelo agregado do BC — existe porque a amostra de estimação começa antes de 2012. **Não é comparável ponto a ponto com `mt_pnad.taxa_desocupacao`**, que sai sem ajuste sazonal: medido no overlap (170 meses), diferença média −0,008 p.p. mas máxima 0,993 p.p., corr 0,9896 — a diferença é sazonal, não metodológica. Recarregada com truncate a cada edição, `vintage` registra de qual | 2004-04 → mês de referência da edição (mensal) | `brasil/bcb/mt_desocupacao_retro.py` |
...
| `br_interest_rate` | **Quatro curvas, duas fontes.** As tres de mercado vem da **B3**, `TaxaSwap.txt` do arquivo de pregao (um arquivo por dia util, livre, sem autenticacao): `DIPRE` (cod `PRE`, DI×pré, 1M–240M), `PREJS` (cod `095`, NTN-F, 6M–120M) e `NTNBJS` (cod `076`, NTN-B, 3M–240M — **e** a taxa indicativa da ANBIMA, bate na 4ª decimal, o que resolve de graça o histórico que a ANBIMA só vende); `NTNBJS` é juro REAL e vai a negativo. Substituíram em 2026-09-03 a `base_mercado.interest_rates` do CentralManagement, cujas 106.308 linhas foram **descartadas** — aquele conector extrapolava sem limite sobre os títulos ofertados no Tesouro Direto (PREJS@240M chegou a −30% a.a.). **Regra de carga: um vértice só existe se o alvo em dias corridos cair DENTRO da grade publicada naquele pregao** — nunca extrapolamos, e é por isso que a cobertura é desigual: `DIPRE@240M` só desde 2014-08 (58,6%) e `PREJS@120M` só desde 2010-02 (**43% dos meses**, com 2013/2021/2025 vazios — o Tesouro emite NTN-F a cada ~2 anos). Faltam 43 pregoes de feriado estadual de SP (B3 fechada) mais 2020-05-07 (B3 publicou as curvas de DI e nenhuma de título). **O cache nao pode gravar ausencia para o dia corrente**: a B3 solta o arquivo depois do fechamento e ate la devolve um zip vazio, identico ao de feriado — cachear isso congela o pregao, porque a janela retroativa le o cache e nunca rebusca. Aconteceu: a tarefa agendada das 09:30 envenenava o proprio dia todo dia e a curva ficou parada em 14/09/2026 por 6 pregoes, com dado real na fonte o tempo todo e nenhum erro. Guarda em `grades()` e em `tests/test_curvas_juros_b3.py`. O guarda `problemas_do_pregao` roda na carga e reporta sem bloquear. A quarta curva é **`POLICY`** (2026-09-03, para centralizar juro de curva e juro definido pelo BC na mesma tabela): a meta de política, vértice `1d`, **materializada** de `macro_international.cmb_policy_rates` (BIS) — concorda com a SGS 432 em **99,98%** dos dias comuns, as 2 divergências sendo dias de reunião do Copom (efetividade D vs D+1); para precisão de dia de decisão a autoridade segue sendo `pm_copom_reuniao`. **Calendário próprio**: vigora em dia corrido, então um `SELECT` num sábado devolve só a POLICY | mercado 2006-01-02 → today (diária); POLICY 1994-07-04 → today (mas a meta Selic só existe desde 1999-03) | `brasil/b3/br_interest_rate.py` |
| `cmb_risco_pais` | Bloomberg (manual xlsx export, `BRAZIL CDS USD SR 5Y D14`) | 2001-10 → today (no gaps beyond 109 business days, all pre-2004) | `brasil/bloomberg/cmb_risco_pais.py` |
...
| `inter_interest_rate` | **BIS WS_CBPOL** (diária) como curva `POLICY`, vértice `1d`, para MX/CL/CO/PE/AR — criada vazia na migração de 2026-09-03 e populada no mesmo dia, a pedido do usuário, para que as três tabelas de juros tenham a mesma curva `POLICY` com a mesma semântica. **É a ingestão do BIS** desde a 2ª rodada do mesmo dia: nasceu lendo a `cmb_policy_rates`, e as duas passaram a guardar o mesmo número — medido, as 53.663 linhas daquela tabela eram reproduzidas **exatamente** (dif máxima 0,0, zero órfãs dos dois lados) por esta mais a `POLICY` da `br_interest_rate`, então ela foi removida e o fetch veio para cá. O BR fica fora por regra de schema e é buscado por `br_interest_rate.py`, com a **mesma** chamada de connector e lista de países disjunta — código compartilhado, não número gravado duas vezes. **Estrutura a termo desses países segue sem fonte** — o que existe aqui é só o ponto curto; não confundir "a tabela tem dado" com "a tabela tem curva". BR e US ficam fora por regra de schema (têm tabela própria), não por falta de dado | AR 1993-04 (congelada em 2025-07, o BIS parou), CO 1995-04, CL 1997-02, MX 1998-11, PE 2003-09 → today | `international/bis/inter_interest_rate.py` |
...
`cmb_dollar_index_em`, `cmb_fx_latam`, `cmb_equity_us`, `comm_brent`, `inter_interest_rate`) são
...
| `mt_produtividade` | BLS/MSPC (the `prod2` release, *Productivity and Costs*), via `pr.data.1.AllData` (3.2 MB, the whole history in one request) with the API as an independent vintage check each load (540 cells, max diff 0.0000). **77,126 rows over 282 series** — a closed cartesian product of 6 sectors × 19 measures × 3 durations, and the `series_id` is **11 chars**, not 13 like the CES or 21 like JOLTS. Three things here do not generalise from the monthly tables. **(1) `duracao` is in the key because the source publishes all three readings** — index (2017=100), change from the previous quarter at an annual rate, change from the same quarter a year earlier — as separate series, at two different precisions: the index at **3 decimals** and the rates at 1. Rebuilding a rate from the file's index agrees to 0.0500 p.p. and never more (the rounding of the published rate); rebuilding from the index *printed in the release* errs 1.35 p.p. at worst. **(2) `periodicidade` separates the quarterly grid from the Q05 annual average**, which would otherwise collide with Q01 on 1 January. The annual average exists only for complete years, so `MAX(date)` is always the last quarter — which is what makes the calendar's freshness check work with no extra clause. `connectors/bls.py` did not know the Q05 code before this table: the row was dropped by `dropna` rather than by the aggregate-period filter, the same answer by accident. **(3) Nothing here is additive** — the six sectors contain one another instead of partitioning, and durable + nondurable over manufacturing averages 2.02 because indexes based at 100 never add. `conceito_produto` carries the release's own incomparability warning as data: measures 04/05/14 (value added) exist only for business/nonfarm/nonfinancial, 21/22/23 (sectoral) only for manufacturing, so the merged `produto_real` slug spans two concepts and the column says which. `classe` is a **function of the sector**, not a dimension, and the load raises if that stops being true. Both identities are validated in **ratio** form, never subtraction: the release's own prose writes productivity as output *minus* hours, which is off by up to **15.9 p.p.** (durable manufacturing, 2020 Q3: subtraction says +54.2%, published is +39.5%) | 1947-Q1 → today (quarterly; 1987-Q1 for manufacturing). Annual averages 1947 → last complete year | `us/labor_market/mt_produtividade.py` |
...
| `expc_inflacao` | **FRED**, serie `EXPINF{n}YR` do **Federal Reserve Bank of Cleveland** (modelo Haubrich-Pennacchi-Ritchken): estrutura a termo da inflacao ESPERADA, 14 vertices (1-10, 15, 20, 25, 30 anos), **mensal**, carimbada no dia 1 do mes de referencia. E a contraparte americana da Focus. Existe porque o juro real **ex-ante** de 2 anos nao tem taxa negociada em mercado: **o Treasury nao publica TIPS de 2 anos** — medido, `DFII2` nao existe no FRED e a curva de TIPS comeca em `DFII5`. O vertice de 10 anos poderia sair do `DFII10`; nao sai, e a razao foi medida antes de decidir: nas 285 medias mensais de 2003-01 a 2026-09, `DGS10 − EXPINF10YR` contra `DFII10` da |erro| medio **0,258 p.p.** e **maximo 1,369 p.p.** (nov/2008, colapso de liquidez dos TIPS), com correlacao de **niveis 0,944** mas de **variacao mensal so 0,699** — e quem consome colore pela variacao. Um metodo so nos dois vertices, e o custo dito no cartao de definicao. **E modelo, nao pesquisa**: o Cleveland reestima e revisa o historico recente, por isso a rotina reescreve 6 meses em vez de so acrescentar o mes novo. A carga levanta se algum mes vier com curva incompleta | 1982-01 → today (mensal) | `us/inflation/expc_inflacao.py` |
| `us_interest_rate` | **FRED**, via `connectors/fred.py`. Duas curvas. `US_TREASURY` sao os **11 vertices** "constant maturity" que o Treasury publica ja no vertice redondo (1M 3M 6M 1Y 2Y 3Y 5Y 7Y 10Y 20Y 30Y; `DGS1MO`..`DGS30`) — nada e interpolado aqui, ao contrario do lado brasileiro. `POLICY` e a **META** do Fed no vertice `1d`, nao a Fed funds efetiva (`DFF`, que e onde o mercado negociou): vem de `DFEDTAR` ate 2008-12-15 e do **ponto medio** de (`DFEDTARL`+`DFEDTARU`)/2 depois, sem um dia de sobreposicao — desde dez/2008 o Fed anuncia um **intervalo** de 25 bps, e o ponto medio e reducao nossa, nao numero publicado (o script levanta se as duas eras passarem a se sobrepor). Recarregada em 2026-09-03: a tabela vinha do CentralManagement com **2 vertices** (DGS2/DGS10, desde 2016) e nenhum consumidor. Os rotulos de `tenor` sao os do proprio Treasury, **em anos** a partir de 1Y — para cruzar com o Brasil use `tenor_years`, nunca o texto. Calendario: a meta vigora em dia corrido, a curva so em dia util | US_TREASURY 1962-01-02 (1Y/3Y/5Y/10Y/20Y; o 1M so em 2001-07) → today; POLICY 1982-09-27 → today | `us/rates/us_interest_rate.py` |
...
PRIMARY KEY (date)                        -- mt_desocupacao_retro (serie unica: a aba publica UMA
                                          --  coluna. `vintage` fica fora da PK, e so procedencia --
                                          --  a tabela guarda uma edicao por vez, como pm_hiato_produto)
...
-- Estrutura a termo: uma linha por dia, curva e vertice
-- `tenor_years` fica FORA da PK de proposito -- e derivado de `tenor` (meses/12) e
-- serve para interpolar e ordenar; o texto de `tenor` e o que a fonte nomeia. As
-- tres tabelas homonimas de macro_us e macro_international tem a mesma forma, com
-- `country_code` na chave da international (BR e US NAO entram la).
PRIMARY KEY (date, curve, tenor)          -- macro_brasil.br_interest_rate, macro_us.us_interest_rate
PRIMARY KEY (date, tenor)                 -- macro_us.expc_inflacao (uma curva so, entao sem `curve`)
                                          -- a curva POLICY entra pelo vertice '1d', com
                                          --  tenor_years = 1/252 e NAO zero: a coluna existe
                                          --  para interpolar/ordenar, e um vertice em zero
                                          --  quebra qualquer interpolacao ancorada no curto
PRIMARY KEY (date, country_code, curve, tenor) -- macro_international.inter_interest_rate
```
